...
public class SamlAuthenticator extends Authenticator {
	...
	if (form.getFirst("SAMLResponse") == null) {
		System.out.println("no saml response, redirect to saml login page");
	    // no SAMLResponse parameter found, redirect to SAML-login page
	    redirectToSamlIdp(request, response);
	} else {
		System.out.println("saml response found");
		...
		//TODO werkt dit?
		String inResponseToId = samlResponse.getInResponseToId();
		request.setResourceRef(SamlStore.get(inResponseToId).getResourceRef());
    ...
    private void redirectToSamlIdp(Request request, org.restlet.Response response) {
	...
	SamlStore.put(authReq.getId(), request);
